...
$(function() {
	...
	$("#userForm").Validform({
		...
		beforeSubmit : function(curform) {
			var options = {
				url : pageContext + "/customer/updateUser",
				type : 'post',
				dataType : 'script',
				async : false,
				success : function() {
						...
						$
								.ajax({
									...
									success : function(data) {
										...
										var text = '选择代理商<select name="agentId" id="agentId"></select> &nbsp;&nbsp;使用代理商logo:<input type="radio" name="useAgentLogo" value="1"  checked />是  <input type="radio" name="useAgentLogo" value="0" />否 '
						...
						if (id == 2) {
							...
							$
									.ajax({
										...
										async : false,
										url : pageContext
												+ "/customer/baseclassList/"	+ parentCode,
										success : function(data) {
